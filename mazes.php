<?php
const pageTitle = "";
require "./php-includes/header.inc.php";
?>

<!-- START - Header -->
<header class="page-header">
    <div class="container h-100">
        <div class="row h-100 align-items-center">
            <div class="col-12 text-center">
                <h1 class="font-weight-light page-font-big">MAZES</h1>
                <p class="page-font-small">ENDLESS MAP MAZES</p>
            </div>
        </div>
    </div>
</header>
<!-- END - Header -->

<!-- START - Session info -->
<section class="section-text-page">
    <div class="container">
        <div class="row">
            <div class="col-lg-12 text-left pb-2 bordered" data-aos="fade-down-right">
                <h3 class="text-green">NATURE</h3>
                <p class="truncate">
                    Spawned from the Gods of the old world, this gift to mankind provides supplies for all on a regular basis. Is it the treasures within calling your name, or is it the very trees themselves? </p>
                    Average Completion Time: ~5 minutes<br>
                    Difficulty: <span style="color:yellow">Medium</span><br></p>
                <p>
                    Rewards:<br>
                    x1 Nature maze token, x8 Honey Apple<br>
                    x1 Medium Money Bag ($2500), 2 Rabbit's foot<br>
                    x3 Wither rose, x8 Honey blocks<br>
                    x1 Bee nest, x1 Farm Starting Kit</i><br>
                    x1 Fig Leaf (Maze set Leggings)
                </p>
                <img src="./images/naturemaze.png" alt="Nature Maze Map" class="feature-rightimg" height="240">
            </div>

            <div class="col-lg-12 text-right pb-2 bordered" data-aos="fade-left">
                <h3 class="pt-3"><span style="color:grey">STONEBRICK</h3>
                <p class="truncate-right">
                    Carved by the dwarves from deep underground. Forged through rock, soil and earth. This maze provides the curious miner with valuable ores and minerals, should you be able to handle the narrow halls and claustrophobic atmosphere.
                    <br><br> Average Completion Time: ~5 minutes<br>
                    Difficulty: <span style="color:green">EASY</span><br></p>
                <p>
                    Rewards:<br>
                    x1 Stonebrick maze token, x16 Bottle o' Enchanting<br>
                    x8 Diamonds, x8 Emeralds<br>
                    x16 Iron ingots, x16 Gold ingots
                </p>
                <img src="./images/stonemaze.png" alt="Stonebrick Map" class="feature-leftimg" height="240">
            </div>

            <div class="col-lg-12 text-left pb-2 bordered" data-aos="fade-right">
                <h3 class="text-green pt-3"><span style="color:blue">SKY</span></h3>
                <p class="truncate">
                    The skies are howling. There's something strange in the air and it's causing a stir. Some say there's a labrynth or tunnels at the top of the water spire. Many people go in, but not many people come out the same. This maze will push you to the brink of insanity.
                    <br><br> Average Completion Time: ~5 minutes<br>
                    Difficulty: <span style="color:red">Hard</span><br></p>
                <p>
                    Rewards:<br>
                    x1 Sky maze token, x32 Gold ingot<br>
                    x32 Nether Quartz, x2 Phantom Membrane<br>
                    x2 Golden Apple (not notch ones), x1 Medium Money Bag ($2500)<br>
                    x1 Angelic Wings (Maze set Elytra)
                </p>
                <img src="./images/skymaze.png" alt="Sky Maze Map" class="feature-rightimg" height="240">
            </div>

            <div class="col-lg-12 text-right pb-2 bordered" data-aos="fade-left">
                <h3 class="text-green pt-4"><span style="color:red">VOLCANO</span></h3>
                <p class="truncate-right">
                    Krakatoa incarnate. This monsterous mountain devours all which has the misfortune of touching the heat of a thousand suns. Civilzation once stood where the gigantic beast now rests. A maze with some serious heat.
                    <br><br> Average Completion Time: ~8 minutes<br>
                    Difficulty: <span style="color:yellow">Medium</span><br></p>
                <p>
                    Rewards:<br>
                    x1 Stonebrick maze token, x16 Bottle o' Enchanting<br>
                    x8 Diamonds, x8 Emeralds<br>
                    x16 Iron ingots, x16 Gold ingots
                </p>
                <img src="./images/volcano.png" alt="Volcano Map" class="feature-leftimg" height="240">
            </div>

            <div class="col-lg-12 text-left pb-2 bordered" data-aos="fade-right">
                <h3 class="text-green pt-4"><span style="color:black">VOID</span></h3>
                <p class="truncate">
                    The Void is never far, and this time it's come the Overworld. With a pitch black environment, endless hallways, and confusing twists and turns. This maze has give many lost wanderers a place to call home.
                    <br><br><br> Average Completion Time: ~10 minutes<br>
                    Difficulty: <span style="color:red">Hard</span><br></p>
                <p>
                    Rewards:<br>
                    x1 Abyss maze token, x1 Potion of night vision 3min<br>
                    x1 Potion of night vision 8min, x2 Phantom membrane<br>
                    x8 Nether rose, x3 Wither skulls<br>
                    x4 Netherite ingots, x1 Medium Money Bag ($2500)<br>
                    x1 Music Disc (Stal), x1 Abyssal Helmet (Maze set Helmet)
                </p>
                <img src="./images/voidmaze.png" alt="Void Maze Map" class="feature-rightimg" height="240">
            </div>

            <div class="col-lg-12 text-right pb-2 bordered" data-aos="fade-left">
                <h3 class="text-green pt-4"><span style="color:blue">ICE</span></h3>
                <p class="truncate-right">
                    A glacier old as time itself. Lasting through all ages and as undying as the universe. This permafrost is a abyss of death. Only the brave may enter here.
                    <br><br><br> Average Completion Time: ~7 minutes<br>
                    Difficulty: <span style="color:yellow">Medium</span><br></p>
                <p>
                    Rewards:<br>
                    x1 Ice maze token, x24 Soul torches<br>
                    x24 Warped stem, x16 Diamonds<br>
                    x32 Packed ice, x16 Snow blocks<br>
                    x1 Ice Maze Pickaxe (Eff5 Unbreak3)
                </p>
                <img src="./images/icemaze.png" alt="Ice Maze Map" class="feature-leftimg" height="240">
            </div>

        </div>
    </div>
</section>
<!-- END - Session info -->

<?php
require "./php-includes/footer.inc.php";
?>
